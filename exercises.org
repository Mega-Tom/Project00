...
**** rassoc
  Takes a value and returns an assocation.
***** Example
#+BEGIN_SRC emacs-lisp :results silent
    (rassoc '(0 0 255)
                 '(("red" 255 0 0) ("green" 0 255 0) ("blue" 0 0 255)))
#+END_SRC
**** alist-get
   Takes a key and returns only the value. Also, compares using =eq= insted of =equal=.
***** Example
#+BEGIN_SRC emacs-lisp :results silent
 (alist-get :abb
    '(
...
*** Your answer goes here.
    You will have your keys compared using strict equality. For example, two
 strings with the same letters could be difrent keys.
...
1. =null= is a predicit that checks for an empty list. Same as =not=.
2. =append= preforms list concatination.
3. =list= makes a list of all its paramiters.
4. =funcall= takes a symbol and runs the assosiated function.
5. =first= is the same as car.
6. =rest= is the same as cdr.

So the program is basicly just:

#+BEGIN_SRC emacs-lisp :results silent
  (defun morphifyr (fun lst)
    (if lst
      (cons (funcall fun (car lst)) (morphifyr fun (cdr lst)))
      nil))
#+END_SRC

This is a basic recersive function that will:
  apply the function to the first element,
  map the rest of the list,
  and join it back together into one list.
...
Almost. It returns a float, while it should return an integer.
...
3. f(n) = (n^2+n)/2-n+3
4. f(n) = n in binary
5. f(n) = f(n-1) + f(n-2); f(1)=f(2)=1
6. f(n) = f(n-1)%2 ? 3*f(n-1) : f(n)/2
7. f(n) = f(n-1) + f(n-2); f(1)=2; f(2)=1
8. f(n) = 3*2^n
9. f(n) = 2*3^n
10. f(n) = nth prime
